...
public class ActionImpl extends EObjectImpl implements Action {
	...
	public LocalInteraction getIs() {
		if (is != null && is.eIsProxy()) {
			...
			is = (LocalInteraction)eResolveProxy(oldIs);
			if (is != oldIs) {
				if (eNotificationRequired())
					eNotify(new ENotificationImpl(this, Notification.RESOLVE, BehaviorPackage.ACTION__IS, oldIs, is));
			}
		}
		return is;
	}
	 ...
	 * @generated
	 */
	public LocalInteraction basicGetIs() {
		return is;
	}
	...
	public void setIs(LocalInteraction newIs) {
		...
		is = newIs;
		if (eNotificationRequired())
			eNotify(new ENotificationImpl(this, Notification.SET, BehaviorPackage.ACTION__IS, oldIs, is));
	}
